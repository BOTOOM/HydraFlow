...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final pickedEntries = picked == null
        ? <IntakeEntry>[]
        : intake.entries.where((entry) => _sameDay(entry.timestamp, picked)).toList();
    return SingleChildScrollView(
      padding: const EdgeInsets.all(20),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Historial', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w800)),
          ...
          child: Padding(
            ...
            child: SizedBox(
              ...
              child: LayoutBuilder(builder: (context, constraints) {
                final max = intake.todayGoal * 1.15;
                final goalFraction = (intake.todayGoal / max).clamp(0.0, 1.0);
                return Stack(children: [
                  Positioned(
                    left: 0,
                    right: 0,
                    bottom: constraints.maxHeight * goalFraction + 24,
                    child: Row(children: [
                      const Expanded(child: Divider(color: HydraTheme.coral)),
                      Text(' meta', style: TextStyle(color: HydraTheme.coral, fontSize: 11)),
                    ]),
                  ),
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: days.map((day) {
                        ...
                        final barHeight = (total / max * height).clamp(4.0, height);
                        final selected = selectedDay != null && _sameDay(selectedDay!, day);
                        return Expanded(
                          child: GestureDetector(
                            ...
                            child: Padding(
                              ...
                              child: Column(mainAxisAlignment: MainAxisAlignment.end, children: [
                                Text('${(total / intake.todayGoal * 100).round()}%', style: const TextStyle(fontSize: 10)),
                                AnimatedContainer(
                                  duration: const Duration(milliseconds: 180),
                                  height: barHeight,
                                  decoration: BoxDecoration(
                                    color: selected ? HydraTheme.coral : HydraTheme.aqua,
                                    borderRadius: BorderRadius.circular(8),
                                  ),
                                ),
                                const SizedBox(height: 6),
                                Text(DateFormat('E', 'es').format(day).substring(0, 2), style: const TextStyle(fontSize: 11)),
                              ]),
                            ),
                          ),
                        );
            ...
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (pickedEntries.isEmpty)
                  const Padding(padding: EdgeInsets.symmetric(vertical: 16), child: Text('No hay registros ese día.'))
                else
                  ...pickedEntries.map((entry) {
                    final drink = drinkById(entry.drinkId, profile.customDrinks);
                    return ListTile(
                      contentPadding: EdgeInsets.zero,
                      leading: Icon(drink.icon, color: drink.color ?? HydraTheme.aqua),
                      title: Text(drink.name),
                      subtitle: Text(DateFormat('HH:mm').format(entry.timestamp)),
                      trailing: Text(formatVolume(entry.effectiveMl, profile.unit)),
                    );
                  }),
              ]),
            ),
